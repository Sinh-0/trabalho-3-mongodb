...
# 2. READ ALL (Com a correção do bug de versão)
    ...
    async def carregar_dados_escala(itens: list[Escala]):
        for escala in itens:
            ...
            if escala.equipe:
                ...
                if ids:
                    funcionarios = await Funcionario.find(Funcionario.id.in_(ids)).to_list()
                    escala.equipe = funcionarios
        return itens

    return await paginate(Escala.find_all(), transformer=carregar_dados_escala)

# 3. READ ONE (Com a correção manual dos links)
...
async def obter_escala(id: PydanticObjectId):
    ...
    # Preencher Setor Manualmente
    if escala.setor and escala.setor.ref.id:
        escala.setor = await Setor.get(escala.setor.ref.id)
    
    # Preencher Equipe Manualmente
    if escala.equipe:
        ids = [link.ref.id for link in escala.equipe if link.ref.id]
        if ids:
            escala.equipe = await Funcionario.find(Funcionario.id.in_(ids)).to_list()
